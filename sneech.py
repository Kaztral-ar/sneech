...
def apt_install(packages):
    """
    Install packages using the correct package manager for the platform.
    - Termux (Android): pkg install  (no sudo, apt-get is unavailable)
    - Linux:            sudo apt-get install
    `packages` is a space-separated string, e.g. "nmap" or "reaver pixiewps".
    """
    if IS_TERMUX:
        run(f"pkg install -y {packages}")
    else:
        run(f"sudo apt-get install -y {packages}")
...
def tool_nmap():
    ...
    if choice == "2":
        confirm("Install Nmap?") and apt_install("nmap")
        return
...
def tool_masscan():
    ...
    choice = ask("(1) Run scan  (2) Install via apt")
    if choice == "2":
        confirm("Install Masscan?") and apt_install("masscan")
        return
...
def tool_dnsenum():
    ...
    confirm("Install dnsenum?") and apt_install("dnsenum")
...
def tool_theharvester():
    ...
    if confirm("Install theHarvester?"):
        if IS_TERMUX:
            # Termux has no sudo/apt-get; install via pip
            run("pip install theHarvester 2>/dev/null || pip3 install theHarvester")
        else:
    ...
    # On Termux the pip-installed entry point is 'theHarvester' (capital H)
...
def tool_ncrack():
    ...
    confirm("Install Ncrack?") and apt_install("ncrack")
    t = sanitize(ask("Target  e.g. ssh://192.168.1.1"))
    if t: run(f"ncrack {t}")

def tool_hydra():
    info("Hydra \u2014 Fast online password cracker.")
    confirm("Install Hydra?") and apt_install("hydra")
...
def tool_hashcat():
    ...
    confirm("Install Hashcat?") and apt_install("hashcat")
...
def tool_john():
    ...
    confirm("Install John?") and apt_install("john")
...
def tool_crunch():
    ...
    confirm("Install Crunch?") and apt_install("crunch")
...
def tool_reaver():
    ...
    if confirm("Install Reaver & dependencies?"):
        apt_install("reaver pixiewps aircrack-ng")

def tool_pixiewps():
    info("Pixiewps \u2014 Offline WPS pixie-dust attack.")
    confirm("Install Pixiewps?") and apt_install("pixiewps")
...
def tool_aircrack():
    ...
    confirm("Install Aircrack-ng?") and apt_install("aircrack-ng")
...
def tool_wifite():
    ...
    confirm("Install Wifite?") and apt_install("wifite")
    run("sudo wifite")
...
def tool_beef():
    ...
    confirm("Install BeEF?") and apt_install("beef-xss")
    run("sudo beef-xss")
...
def tool_sslstrip():
    ...
    if confirm("Clone & install SSLstrip?"):
        ...
        apt_install("python3-twisted" if not IS_TERMUX else "python-twisted")
        run("cd sslstrip && sudo python3 setup.py install")

def tool_wireshark():
    info("Wireshark \u2014 Graphical packet analyser.")
    confirm("Install Wireshark?") and apt_install("wireshark")
    run("sudo wireshark &")

def tool_ettercap():
    info("Ettercap \u2014 Comprehensive MITM suite.")
    confirm("Install Ettercap?") and apt_install("ettercap-graphical")
    run("sudo ettercap -G &")

def tool_bettercap():
    info("Bettercap \u2014 Swiss army knife for network attacks.")
    confirm("Install Bettercap?") and apt_install("bettercap")
    run("sudo bettercap")

def tool_tcpdump():
    info("tcpdump \u2014 CLI packet capture.")
    confirm("Install tcpdump?") and apt_install("tcpdump")
...
def tool_arpspoof():
    ...
    confirm("Install dsniff (provides arpspoof)?") and apt_install("dsniff")
...
def tool_wpscan():
    ...
    if confirm("Install WPScan?"):
        apt_install("ruby")
        gem_cmd = "gem install wpscan" if IS_TERMUX else "sudo gem install wpscan"
        run(gem_cmd)
...
def tool_nikto():
    ...
    confirm("Install Nikto?") and apt_install("nikto")
    target = sanitize(ask("Target URL"))
    if target: run(f"nikto -h {target}")

def tool_whatweb():
    info("WhatWeb \u2014 Web technology fingerprinting.")
    confirm("Install WhatWeb?") and apt_install("whatweb")
...
def tool_feroxbuster():
    ...
    if confirm("Install Feroxbuster?"):
        if IS_TERMUX:
            # Feroxbuster is a Rust binary; install via cargo on Termux
            run("pkg install -y rust && cargo install feroxbuster")
        else:
...
def tool_gobuster():
    ...
    confirm("Install Gobuster?") and apt_install("gobuster")
    target = sanitize(ask("Target URL"))
...
def tool_xsser():
    ...
    confirm("Install XSSer?") and apt_install("xsser")
    target = sanitize(ask("Target URL"))
